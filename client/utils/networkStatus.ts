...
export interface NetworkError {
  type: 'timeout' | 'fetch_failed' | 'server_down' | 'invalid_response' | 'unknown';
  message: string;
  userMessage: string;
  shouldRetry: boolean;
}
...
export const parseNetworkError = (error: any): NetworkError => {
  ...
  if (errorMessage.includes('Network error')) {
    return {
      type: 'server_down',
      message: errorMessage,
      userMessage: 'The server appears to be unavailable. Please try again later.',
      shouldRetry: true,
    };
  }

  if (errorMessage.includes('JSON') || errorMessage.includes('HTML instead of JSON')) {
    return {
