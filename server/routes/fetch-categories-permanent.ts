...
export const fetchCategoriesPermanent: RequestHandler = async (req, res) => {
  try {
    ...
    res.write(
      `🚀 Fetching and permanently saving ${categories.length} business categories...\n\n`,
    );
        ...
        if (data.status === "OK" && data.results) {
          ...
          for (const business of businesses) {
            try {
              ...
              if (!existing) {
                // Save business with correct schema
                await database.run(
                  `
                ...
                `,
                  [
                    ...
                    `${business.types?.join(", ") || ""} - Found via ${category} search`,
    ...
    const categoriesResult = await database.get(
      "SELECT COUNT(DISTINCT category) as total FROM businesses",
    );
    ...
    res.write(`📂 Total categories: ${categoriesResult.total}\n`);
    ...
    res.write(`\n🏢 Categories permanently saved to database!`);
    res.end();
  } catch (error) {
    console.error("Permanent fetch error:", error);
    if (!res.headersSent) {
